...
buscaSigToken "" "" =("$","")
buscaSigToken "" y = if((ord(head y)>=ord('0') && ord(head y)<=ord('9'))||ord(head y)==ord('-')) then ("ZENB",y) else if (rtrim y)`elem` ["mozketa","biraketa","islapena","leku_aldatzea","hautazko_ardatz_baten_inguruko_biraketa"] then ((palabra_clave (rtrim y)),(rtrim y)) else ("ID",y)
buscaSigToken x y |((head x)=='\n'||(head x)==' ') && (trim y)==""=(buscaSigToken  (tail x) "")
                  ...
                  |otherwise = if((ord(head y)>=ord('0') && ord(head y)<=ord('9'))||ord(head y)==ord('-')) then ("ZENB",y) else if (rtrim y)`elem` ["mozketa","biraketa","islapena","leku_aldatzea","hautazko_ardatz_baten_inguruko_biraketa"] then ((palabra_clave (rtrim y)),(rtrim y)) else ("ID",y)
