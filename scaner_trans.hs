...
palabra_clave x |(x=="mozketa")="MOZKETA"
				...
				|(x=="spline")="SPLINE"
				|(x=="bezier")="BEZIER"
				|(x=="nahaste_parab")="NAHASTE_PAR"
				|(x=="bspline")="BSPLINE"
				|(x=="bspline2")="BSPLINE_IRE"
				|(x=="biratu")="BIRATU_OSOA"
				|(x=="ekorketa_gainazala")="EKORKETA"
buscaSigToken::String->String->(String,String)
buscaSigToken "" "" =("$","")
buscaSigToken "" y = if((ord(head y)>=ord('0') && ord(head y)<=ord('9'))||ord(head y)==ord('-')) then ("ZENB",y) else if (rtrim y)`elem` ["mozketa","biraketa","islapena","leku_aldatzea","hautazko_ardatz_baten_inguruko_biraketa","hiperbola","parabola","spline","bezier","nahaste_parab","bspline","biratu","bspline2","ekorketa_gainazala"] then ((palabra_clave (rtrim y)),(rtrim y)) else ("ID",y)
buscaSigToken x y |((head x)=='\n'||(head x)==' ') && (trim y)==""=(buscaSigToken  (tail x) "")
				  ...
				  |otherwise = if((ord(head y)>=ord('0') && ord(head y)<=ord('9'))||ord(head y)==ord('-')) then ("ZENB",y) else if (rtrim y)`elem` ["mozketa","biraketa","islapena","leku_aldatzea","hautazko_ardatz_baten_inguruko_biraketa","hiperbola","parabola","spline","bezier","nahaste_parab","bspline","biratu","bspline2","ekorketa_gainazala"] then ((palabra_clave (rtrim y)),(rtrim y)) else ("ID",y)
